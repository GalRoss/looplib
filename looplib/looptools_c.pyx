...
cpdef get_parent_loops(l_sites, r_sites):
    ...
    """
    cdef cnp.int64_t N = len(l_sites)
    cdef cnp.int64_t[:] sortarg = np.argsort(np.r_[l_sites.astype(np.float64)+0.1,
                                                r_sites.astype(np.float64)-0.1])
    cdef cnp.int64_t[:] looporder = np.take(np.r_[np.arange(1,N+1),-np.arange(1,N+1)],sortarg)
    cdef cnp.int64_t[:] parents = -3 * np.ones(N, dtype=int)

    cdef cnp.int64_t i = 0
    cdef cnp.int64_t curloop = 0
    cdef cnp.int64_t parent = 0
    cdef cnp.int64_t[:] stack = -1 * np.ones(N+1, dtype=int)
    cdef cnp.int64_t stack_level = 0
    for i in range(2*N):
...
cpdef list get_stationary_loops(
    cnp.int64_t[:] prev_lsites,
    cnp.int64_t[:] new_lsites,
    int delta):
    """
